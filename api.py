...
def configure_ngrok():
    ngrok_token = "2pFoC0QXMffbCBzmFupYIPBGFFy_2FCShy5EuTuHxJbJE7Wsf"
    try:
...
def update_soul_txt(public_url):
    with open("anony4.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony4.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony4.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony4.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony4.txt on VPS: {str(e)}")
